    ...
    if ($result->num_rows > 0) {
        ...
        <table>
            <tr>
                ...
                $sql = "UPDATE Richieste SET Stato = -2 WHERE IDRichiesta = '$IDRichiesta'";
                if ($connessione ->query($sql)){
                    echo "<h3>Prenotazione annullata</h3>";
                    //invio email all'admin
                    if(check_login(false)){
                        send_mail('email@admin.com', 'Annulla Prenotazione Libro', 'L\'alunno "'.$_SESSION['USER']->email.'" ha annullato la prenotazione del libro  "' .$row['Titolo'] . '", con id "' .$row['IDLibro']. '" nel periodo che va da "'.$row['DataRitiro'].'" al "'.$row['DataConsegna'].'"');
                    }
                }else{
